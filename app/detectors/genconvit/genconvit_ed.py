...
from app.detectors.genconvit.model_embedder import HybridEmbed
...
class GenConViTED(nn.Module):
    """GenConViT Encoder-Decoder network.

    Architecture: CNN encoder-decoder reconstructs the image, then ConvNeXt
    backbone (with Swin-based HybridEmbed patch embedding) classifies both
    the original and reconstructed images. Features are concatenated and
    passed through FC layers for binary classification.
    """
    ...
    def __init__(self, config, pretrained=False):
        ...
        self.embedder = timm.create_model(
            config["embedder"], pretrained=pretrained
        )
